...
const Quotations = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const quotationData = {
      ...
      quotationDate,
      validUntil,
      subtotal: parseFloat(subtotal), // Ensure subtotal is number
      total: parseFloat(total), // Ensure total is number
      items: items.map(item => ({
        itemId: parseInt(item.itemId),
        unit: item.unit,
        quantity: parseFloat(item.quantity), // Ensure quantity is number
        price: parseFloat(item.price), // Ensure price is number
        discountPercent: parseFloat(item.discountPercent || 0), // Ensure discountPercent is number
        total: parseFloat(item.total), // Ensure item total is number
      ...
      <div className="flex justify-between items-center mb-8">
        ...
        <button 
          onClick={() => {
            ...
            setShowModal(true);
            setEditingQuotation(null); // Ensure no quotation is being edited when opening for creation
            resetForm();
          }}
                ...
                <div className="form-grid-2">
                  ...
                  <div className="form-group">
                    ...
                    <input
                      ...
                      className="form-input"
                      min={formData.quotationDate} // Cannot be before quotation date
                      required
                    />
                  </div>
                  <div className="form-group">
                          ...
                          {formData.items.map((item, index) => (
                            <tr key={index} className="border-b">
                              ...
                              <td className="py-2 px-2">
                                <input
                                  type="number"
                                  name="price" // Corrected: use price here
                                  value={item.price} // Corrected: use price here
                                  onChange={(e) => updateItem(index, 'price', e.target.value)} // Corrected: use price here
                                  className="form-input"
                                  placeholder="0.00"
                                  min="0"
                                  step="0.01"
                              ...
                              <td className="py-2 px-2">
                                <input
                                  type="number"
                                  name="discountPercent" // Corrected: use discountPercent here
